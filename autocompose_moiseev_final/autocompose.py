...
# +решено править руками


#  + группировать или ставить на входной параметр ЛЕЙБЛ СТЭКА
#  + 'Labels': {'com.docker.stack.namespace': 'rpn-backend-develop'
...
def main():
    print("Please manually modify all the networks directives and the block with their declaration: (because not all the read networks are needed to deploy the stack). MoiseevPA)")
    print("Please use stack flag (for example:  --stack rpn-backend-develop). by default, services of all stacks are displayed. MoiseevPA")
...
####moiseev
    parser.add_argument(
        ...
        "--stack",
        #action="store_true",
        help="Include information of containers and services only the target stack",
    )
####
    ...
    if args.stack:
        #print(" МЫ СЕЙЧАС ЗАШЛИ В АРГС.СТАК")
        global STACK_NAME
        STACK_NAME = args.stack  #moiseev

        global FLAG_STACK
        if STACK_NAME != "print all":
            FLAG_STACK = True
    ...
    for cname in container_names:  # здесь цикл заполнения yml-файла  ПРИЧЕМ здесь приходит айдишник. но заложено что может и имя какое то
...
def placement_constraints_moiseev(container_name):  # может можно будет в будущем вызывать единожды
    ...
    #print("вывод   container_name  когда перешли в функцию placement_constraints_moiseev")
    #print(container_name)
    placement_constraints = ""
    try:  #ЕСЛИ   ЗАПУСК С WORKER ТО НЕТ ДОСТУПА ДО СЕРВИСОВ
        resized_container_name = container_name.partition('.')[0]  # отрезаем имя после точки и точку

        #c = docker.from_env()
        services_id_list = CLIENT.services.list()  # список всех айди сервисов
        ...
        for i in list_range:
            ...
            service_id = services_id_list[i].short_id

            #print("вывод service_id short_id")
            #print(services_id_list[i].short_id)
            #print(service_id)
            #print("_______")

            services_id_list_filtered[i] = service_id
            services_name_list[i] = CLIENT.services.get(service_id).name  # список всех имен этих сервисов
        ...
        for i in list_range:
            ...
            # print("вывод resized_container_name")
            # print(resized_container_name)
            # print("   ")
            #
            # print("вывод services_name_list[i]")
            # print(services_name_list[i])
            # print("   ")
            ...
            if resized_container_name == services_name_list[i]:
                ...
                break
        ########################################################################
        # print("вывод target_service_id")
        # print(target_service_id)
        #
        # print("вывод исходный service_id")
        # print(service_id)

        if target_service_id != "":
            sattrs = CLIENT.services.get(target_service_id).attrs

            #print("ry8tputhdd6m ry8tputhdd6m ry8tputhdd6m ry8tputhdd6m ry8tputhdd6m ry8tputhdd6m ry8tputhdd6m")
            #print (CLIENT.services.get("ry8tputhdd6m").attrs)



            placement_constraints = sattrs.get("Spec", {}).get("TaskTemplate", {}).get("Placement", {}).get("Constraints", {})
        else:
            print("контейнер  ->  " + container_name + "  <-  НЕ БЫЛ НАЙДЕН в списке сервисов по короткому имени. ")
    except Exception as e:
        #docker.errors.APIError: 503 Server Error: Service Unavailable
        #print("Возможно, эта нода не является менеджером. Ошибка: ")
        #print('Ошибка:\n', traceback.format_exc())
        if traceback.format_exc().find("This node is not a swarm manager"):
            placement_constraints = "node.role == worker"
            #print("Данная нода ТОЧНО не является менеджером. Прописываем node.role == worker")
        pass
    #finally:
    #   pass
...
#moiseev тестовое Исключение
    try:
        container_name = CLIENT.containers.get(cname).name
        #print("container_name")
    except IndexError:
        print("No such container. ВОЗМОЖНО, Swarm перезапускал контейнер в момент работы программы", file=sys.stderr)
        sys.exit(1)
##moiseev тестовое Исключение

    #print("СЕРВИС " + container_name + " +++++ прошли проверку IF на флаги и неймспейс+++++  заполняем данные по сервису")
    #print(placement_constraints_moiseev(container_name))
    ...
    if container_name[:9] != "portainer" or container_name.find(
        ...
        #print(" ВЫВОД com.docker.stack.namespace")
        #print(cattrs.get("Config", {}).get("Labels", {}).get("com.docker.stack.namespace", None))

        stack_namespace = cattrs.get("Config", {}).get("Labels", {}).get("com.docker.stack.namespace", None)
        if stack_namespace is not None:
            #print("+++ ИМЯ СТЭКА НЕ NONE")
            #print("FLAG_STACK   " + str(FLAG_STACK))
            #print("STACK_NAME   " + STACK_NAME)

            if (FLAG_STACK == False and STACK_NAME == "print all") or (FLAG_STACK == True and STACK_NAME == stack_namespace):
                ...
                #         networks = None
                # else:
                #    networklist = CLIENT.networks.list()    # если сети заполнены, читаем список сетей
                ...
                for key in values:
                    ...
                    if value not in IGNORE_VALUES:
                        ct[key] = value
                    #print("+++заполняем данные для данного сервиса cfile, networks, volumes ")
                    #print("constraints  -ВЫВОДИМ   " + str(placement_constraints_moiseev(container_name)))
                    #print(" ____________")

                return cfile, networks, volumes

            else:
                #print("почему то вошли в 1ый ELSE   -  если не тот СТЭК")
                return "", "", "" # отдаем пустое если не тот СТЭК
        else:
            #print("почему то вошли в 2й ELSE  --     если СТЭК NONE  тип")
            return "", "", ""  # отдаем пустое если СТЭК NONE тип
    else:
        #print("почему то вошли в 3й ELSE     -это ПОРТЕЙНер")
        #print(" ____________")
